...
public class ThalmicMyo : MonoBehaviour {
    ...
    void Update() {
        ...
        lock (_lock) {
			...
			myoSynced = armSynced;//update status
			EmgData = emg;
			gyrox = accelerometer.x;
			gyroy = accelerometer.y;
			gyroz = accelerometer.z;
			accx = gyroscope.x;
			accy = gyroscope.y;
			accz = gyroscope.z;
        }
		...
		if (isPaired) 
		{
			...
			for(int i=0; i<=emg.Length-1; i++)
			{
				//accel
				if(!DevicesLists.availableDev.Contains("MYO:ANALOG:EMG:SENSORS"))
				{
					DevicesLists.availableDev.Add("MYO:ANALOG:EMG:SENSORS");		
				}
				if(DevicesLists.selectedDev.Contains("MYO:ANALOG:EMG:SENSORS") && UDPData.flag)
				{
					UDPData.sendString("[$]analog,[$$]"+"myo"+",[$$$]"+"emg,"+"sensor"+(i+1)+","+emg[i].ToString()+";"); 
				}
			}

			if(!DevicesLists.availableDev.Contains("MYO:TRACKING:ACCELEROMETER:POSITION"))
			{
				DevicesLists.availableDev.Add("MYO:TRACKING:ACCELEROMETER:POSITION");		
			}
			if(DevicesLists.selectedDev.Contains("MYO:TRACKING:ACCELEROMETER:POSITION") && UDPData.flag==true)
			{					
				//gyro and accell reversed 
				UDPData.sendString("[$]analog,[$$]"+"myo"+",[$$$]"+"accelerometer,"+"acceleration,"+gyroscope.x.ToString()+","+gyroscope.y.ToString()+","+gyroscope.z.ToString()+";");  
			}

			if(!DevicesLists.availableDev.Contains("MYO:TRACKING:GYROSCOPE:ORIENTATION"))
			{
				DevicesLists.availableDev.Add("MYO:TRACKING:GYROSCOPE:ORIENTATION");		
			}
			if(DevicesLists.selectedDev.Contains("MYO:TRACKING:GYROSCOPE:ORIENTATION") && UDPData.flag==true)
			{					
				//gyro and accell reversed 
				UDPData.sendString("[$]analog,[$$]"+"myo"+",[$$$]"+"gyroscope,"+"orientation,"+accelerometer.x.ToString()+","+accelerometer.y.ToString()+","+accelerometer.z.ToString()+";"); 
			}
	...
	public static bool myoSynced; 

	public static int[] EmgData;
	...
	void OnGUI ()
	{
		if(MainGuiControls.MyoMenu)
		{
			GUI.Label (new Rect (Screen.width-120, 50, Screen.width, Screen.height), "EMG");
			//print EMG
			if (isPaired) {
			for(int i=0; i<=emg.Length-1; i++)
			{
				GUI.Label (new Rect (Screen.width-120, 100+(i*20), Screen.width, Screen.height), (i+1)+": "+ emg[i].ToString());
			}

			GUI.Label (new Rect (Screen.width-200, 280, Screen.width, Screen.height), "Gyro: "+accelerometer.ToString());
			GUI.Label (new Rect (Screen.width-200, 310, Screen.width, Screen.height), "Acc: "+gyroscope.ToString());
