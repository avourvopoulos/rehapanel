...
public class CalibCamera : MonoBehaviour, IGazeListener, ICalibrationProcessHandler
{
    ...
    {
		//-----------------------------------------
//		if (GazeManager.Instance.IsCalibrating && !Screen.fullScreen)
//		{
//			Screen.fullScreen = true;
//			Debug.Log("Calibration Fullscreen");
//		} 
//		else 
//		{
...
//        }
    ...
    void OnGUI()
    {
		if(MainGuiControls.TETMenu && TETSettings.eyeServer)
		{
	        ...
	        if (numBtns > 0)
	        {
	            ...
	            if (GazeManager.Instance.IsCalibrated)
	            {
	                String btnText = "Start Gaze";
	
	                if (GUI.Button(new Rect(x + btnPadding, y + btnPadding, btnWidth, btnHeight), btnText))
	                {
	                    //Application.LoadLevel(1);
						// Application.LoadLevelAdditive("std_scene");
						TETSettings.isConnected = true;
...
//        if (GUI.Button(new Rect(padding, padding, btnWidth, btnHeight), "Gaze"))
//        {
//            //Application.Quit();
//			Calibration.SetActive(false);
//			Gaze.SetActive(true);	
