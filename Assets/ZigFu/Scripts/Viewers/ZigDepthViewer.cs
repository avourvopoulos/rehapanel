...
public class ZigDepthViewer : MonoBehaviour {
    ...
    void OnGUI() 
	{
		if(MainGuiControls.KinectMenu)
		{
			...
			if (null == target) {
				...
				GUI.DrawTexture(new Rect(Screen.width - texture.width - 230, (Screen.height - texture.height - 35+KinectGUI.resize)*MainGuiControls.hideMenu+MainGuiControls.hideviewers, texture.width, texture.height), texture);
	...
	void Update () 
	{
		if (Zig.trackeduser == true) {
				...
				} else if (Zig.trackeduser == false) {
						//	ZigUsersRadar.skeltrack=false;
						BaseColor = Color.red;
				} else {
